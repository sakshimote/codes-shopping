...
export class WalletServiceService {
  ...
  payBywallet(amount:number, walletId:string):Observable<wallet>{
    return this.http.get<wallet>(this.payByWalletApi+amount+"/"+walletId,{headers: { authorization: this.loginService.createBasicAuthToken(this.loginService.username, this.loginService.password) } });
  }

  getStatement(walletId:string):Observable<OrderStatement[]>{
    return this.http.get<OrderStatement[]>(this.getStatementsApi+walletId,{headers: { authorization: this.loginService.createBasicAuthToken(this.loginService.username, this.loginService.password) } });
  }
}
