...
export class CartService {
  ...
  public getCartByUserId(){
    ...
    return this.http.get<Cart>(this.getCartByUserIdApi+userId,{headers: { authorization: this.loginService.createBasicAuthToken(this.loginService.username, this.loginService.password) } })
  }

  public getCartById(cartId:string){
    return this.http.get<Cart>(this.getByCartIdApi+cartId,{headers: { authorization: this.loginService.createBasicAuthToken(this.loginService.username, this.loginService.password) } });
  }

  public getCarts():Observable<Cart[]>{
    return this.http.get<Cart[]>(this.getCartsApi,{headers: { authorization: this.loginService.createBasicAuthToken(this.loginService.username, this.loginService.password) } });
  }

public addCart() :Observable<Cart> {
 let userId=localStorage.getItem("userId");
  return this.http.post<Cart>(this.addCartApi+userId,{},{headers: { authorization: this.loginService.createBasicAuthToken(this.loginService.username, this.loginService.password) } });
...
return this.http.post<Cart>(this.addProductsApi+userId+"/"+productId+"/"+(1),{},{headers: { authorization: this.loginService.createBasicAuthToken(this.loginService.username, this.loginService.password) } });
}
