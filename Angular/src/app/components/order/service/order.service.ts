...
export class OrderService {
  ...
  addOrder(userId:string,order:Order) : Observable<Order>{
    ...
    return this.http.post<Order>(this.addOrderApi+userId,order,{headers: { authorization: this.loginService.createBasicAuthToken(this.loginService.username, this.loginService.password) } });

  }

  getOrderByUser(userId:string): Observable<Order[]>{
    userId=localStorage.getItem("userId");
    return this.http.get<Order[]>(this.getOrderByUserIdApi+userId,{headers: { authorization: this.loginService.createBasicAuthToken(this.loginService.username, this.loginService.password) } });
